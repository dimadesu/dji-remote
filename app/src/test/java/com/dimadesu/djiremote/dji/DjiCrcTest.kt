...
class DjiCrcTest {
    ...
    // Table-driven CRC8 from DJI-Wifi-Connect Python reference (ble.py)
    // This is an independent implementation to cross-verify our bit-by-bit algo
    ...
    // Table-driven CRC16 from DJI-Wifi-Connect Python reference (ble.py)
    ...
    private fun tableCrc16(data: ByteArray): Int {
        var crc = 0x3692
        for (b in data) {
            crc = (crc ushr 8) xor crc16Table[(crc xor (b.toInt() and 0xFF)) and 0xFF]
        }
        return crc and 0xFFFF
    }

    @Test
    fun testCrc8CrossVerify() {
        // Cross-verify our bit-by-bit CRC8 against the Python reference table
        val testVectors = arrayOf(
            byteArrayOf(0x55, 0x33, 0x04),
            byteArrayOf(0x55, 0x0D, 0x04),
            byteArrayOf(0x55, 0x0B, 0x04),
            byteArrayOf(0x00),
            byteArrayOf(0xFF.toByte()),
            byteArrayOf(0x01, 0x02, 0x03, 0x04, 0x05),
            ByteArray(256) { it.toByte() },
        )
        for (data in testVectors) {
            val ours = DjiCrc.computeCrc8(data)
            val table = tableCrc8(data)
            assertEquals("CRC8 mismatch for ${data.take(5).joinToString { "%02X".format(it) }}", table, ours)
        }
    }

    @Test
    fun testCrc16CrossVerify() {
        // Cross-verify our bit-by-bit CRC16 against the Python reference table
        val testVectors = arrayOf(
            byteArrayOf(
                0x55, 0x33, 0x04, 0x04,
                0x02, 0x07,
        ...
        for (data in testVectors) {
            val ours = DjiCrc.computeCrc16(data)
            val table = tableCrc16(data)
            assertEquals("CRC16 mismatch for ${data.take(5).joinToString { "%02X".format(it) }}", table, ours)
        }
    }
    ...
    fun testCrc8() {
        ...
        assertEquals(0xC2, crc) // Verified against Python reference table
    }
    ...
    fun testCrc16() {
        ...
        val message = byteArrayOf(
            0x55, 0x33, 0x04, 0xC2.toByte(), // Header with CRC8=0xC2
        ...
        // Verified against Python reference table: 0x5ACE (LE: CE 5A)
        assertEquals(0x5ACE, crc16)
    }
